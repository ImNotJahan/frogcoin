...
#if defined(NDEBUG)
# error "Frogcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdeb977622fbce98a9e32297bf50e4e09b47e580c22cd99fe0eeb153d012d3f7e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Frogcoin: starting difficulty is 1 / 2^12
...
{
    // Frogcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Frogcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Frogcoin: 3 days
static const int64 nTargetSpacing = 60; // Frogcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Frogcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Frogcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ecb983c954fa33a4504bac07415651b962adef9516acd9ccaee6115b6405891");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2022/02/14 - Scientists find a few way to catch elephant poachers";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042f03da2287a1ccf7bf072f9a11b12e4dbd84ecbaa279d0229ac55c4e5f361996fa30668236234e9160a37471b785dfc12d4b9ab6d83fa037e2a3ec8c33e746ff") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1644890924;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084832991;

        if (fTestNet)
        {
            block.nTime    = 1644890914;
            block.nNonce   = 385640631;
        }
	...
	if (block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4590f61f49ca021a6860d85925a03a4cbe30b27983f36923b78ef5abc16068f0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf0, 0xce, 0xb2, 0xda }; // Frogcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FrogcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrogcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FrogcoinMiner(CWallet *pwallet)
{
    printf("FrogcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FrogcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FrogcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FrogcoinMiner, pwallet));
}
